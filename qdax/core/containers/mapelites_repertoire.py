...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def add(
        ...
        improvement = (batch_of_fitnesses - jnp.where(current_fitnesses == -jnp.inf, 0., current_fitnesses)) * addition_condition
        ...
        ), addition_condition.reshape(-1), improvement.reshape(-1)
    ...
    def init(
        ...
        new_repertoire, is_offspring_added, improvement = repertoire.add(genotypes, descriptors, fitnesses, extra_scores)

        return new_repertoire, is_offspring_added, improvement  # type: ignore
