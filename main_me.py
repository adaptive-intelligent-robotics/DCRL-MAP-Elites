...
def main(config: DictConfig) -> None:
    ...
    metrics = dict.fromkeys(["iteration", "qd_score", "coverage", "max_fitness", "qd_score_repertoire", "dem_repertoire", "ga_offspring_added", "ga_improvement", "time"], jnp.array([]))
    ...
    for i in range(num_loops):
        ...
        if i == -1:
            current_metrics["ga_offspring_added"] = get_elites(current_metrics["is_offspring_added"] + is_offspring_added)
            current_metrics["ga_improvement"] = get_elites(current_metrics["improvement"] + improvement)
        else:
            current_metrics["ga_offspring_added"] = get_elites(current_metrics["is_offspring_added"])
            current_metrics["ga_improvement"] = get_elites(current_metrics["improvement"])
