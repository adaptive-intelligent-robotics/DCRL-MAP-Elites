...
class QualityDCGGeccoEmitterState(EmitterState):
...
class QualityDCGGeccoEmitter(Emitter):
    """
    ...
    def __init__(
        ...
        self._scoring_actor_fn = scoring_actor_fn
    ...
    def init(
    ...
    ) -> Tuple[QualityDCGGeccoEmitterState, RNGKey]:
        ...
        observation_size = jax.tree_util.tree_leaves(init_genotypes)[1].shape[1]
        ...
        random_key, subkey = jax.random.split(random_key)
        emitter_state = QualityDCGGeccoEmitterState(
    ...
    def emit(
        ...
        """
        # sample parents
        parents, descriptors, random_key = repertoire.sample_with_descs(
            random_key, self._config.env_batch_size)

        # apply the pg mutation
        genotypes = self.emit_pg(emitter_state, parents, descriptors)

        return genotypes, {}, random_key

    @partial(jax.jit, static_argnames=("self",),)
    def emit_pg(
        self, emitter_state: QualityDCGGeccoEmitterState, parents: Genotype, descs: Descriptor) -> Genotype:
    ...
    @partial(jax.jit, static_argnames=("self",))
    def emit_actor(self, emitter_state: QualityDCGGeccoEmitterState) -> Genotype:
    ...
    def state_update(
        ...
        desc = jnp.repeat(descriptors[:, jnp.newaxis, :], episode_length, axis=1)/self._config.max_bd
        transitions_with_desc = transitions.replace(desc=desc, desc_prime=desc)

        # Active transitions
        actors_params = jax.tree_util.tree_map(lambda x: jnp.repeat(jnp.expand_dims(x, axis=0), self._config.actor_batch_size, axis=0),
                                     emitter_state.actor_params)

        repertoire_empty = repertoire.fitnesses == -jnp.inf
        p = (1.0 - repertoire_empty) / jnp.sum(1.0 - repertoire_empty)
        random_key, subkey = jax.random.split(emitter_state.random_key)
        desc_prime = jax.tree_util.tree_map(
            lambda x: jax.random.choice(subkey, x, shape=(self._config.actor_batch_size,), p=p),
            repertoire.descriptors,
        )
        desc_prime = desc_prime.reshape(desc_prime.shape[0], self._env.behavior_descriptor_length)
        _, descriptors, extra_scores, random_key = self._scoring_actor_fn(
            actors_params, desc_prime, random_key
        )

        assert "transitions" in extra_scores.keys(), "Missing transitions or wrong key"
        transitions = extra_scores["transitions"]

        desc_prime = jnp.repeat(desc_prime[:, jnp.newaxis, :], episode_length, axis=1)/self._config.max_bd
        desc = jnp.repeat(descriptors[:, jnp.newaxis, :], episode_length, axis=1)/self._config.max_bd

        transitions_actor_with_desc = transitions.replace(desc=desc, desc_prime=desc_prime)

        # Concatenate transitions
        transitions = jax.tree_util.tree_map(lambda x, y: jnp.concatenate([x, y]), transitions_with_desc, transitions_actor_with_desc)
        ...
        transitions, random_key = replay_buffer.sample(random_key, self._config.num_critic_training_steps*self._config.batch_size)
        ...
        def scan_train_critics(
            carry: QualityDCGGeccoEmitterState, transitions
        ) -> Tuple[QualityDCGGeccoEmitterState, Any]:
            emitter_state = carry
    ...
    @partial(jax.jit, static_argnames=("self",))
    def _train_critics(
        self, emitter_state: QualityDCGGeccoEmitterState, transitions
    ) -> QualityDCGGeccoEmitterState:
    ...
    def _mutation_function_pg(
        ...
        descs_prime = jnp.tile(descs, (self._config.num_pg_training_steps*self._config.batch_size, 1))/self._config.max_bd
        transitions = transitions.replace(rewards=self._similarity(transitions.desc, descs_prime)*transitions.rewards, desc_prime=descs_prime)
        ...
        def scan_train_policy(
            carry: Tuple[QualityDCGGeccoEmitterState, Genotype, optax.OptState],
            transitions,
        ) -> Tuple[Tuple[QualityDCGGeccoEmitterState, Genotype, optax.OptState], Any]:
    ...
    def _train_policy(
        ...
        transitions,
    ) -> Tuple[QualityDCGGeccoEmitterState, Params, optax.OptState]:
